import { use, start, writeResponse } from "./09a-httpFramework.js"
import fs from 'fs'
...
use("GET", "index.html", index)

use("GET", "file", file)

use("GET", "sum", sum);

use("GET", "publicSum", publicSum);

use("GET", "createFile", createFile);

use("POST", "postTest", postTest);

use("POST", "signup", signup);

use("POST", "login", login);

use("GET" , "getRecords" , getRecords)

use("PUT" , "checkRecords" , checkRecords)

use("POST" , "addItem" , addItem)
...
function addItem(response, request) {
    console.log(request.body);
    fs.readFile("./todo.json", { encoding: "utf8" }, (err, body) => {
        if (err) {
            ...
            if (!found) {
                ...
                writeResponse(response, 200,JSON.stringify({result: "task updated!"}))
            }
        }
    });
...
function checkRecords(response, request) {
    console.log(request.body);
    fs.readFile("./todo.json", { encoding: "utf8" }, (err, body) => {
        if (err) {
            console.log("file did not save!");
...
function getRecords(response, request) {
    fs.readFile("./todo.json", { encoding: "utf8" }, (err, body) => {
        if (err) {
            console.log("file did not save!");
        }
            ...
            writeResponse(response, 200,JSON.stringify({result: body.records}))
        }
    });
}
...
function sum(response, request) {
    ...
    if (!verifyToken(parseCookie(request.headers.cookie, "token"))) {
        ...
        console.log(inputs);
        writeResponse(response, 200, JSON.stringify({ "sum:": (Number(inputs[0]) + Number(inputs[1])) }))
    }

}

function publicSum(response, request) {
    let urlArray = request.url.split("/");
    console.log(urlArray);
    let inputs = urlArray.slice(2)
    console.log(inputs);
...
function createFile(response, request) {
    let urlArray = request.url.split("/");
    console.log(urlArray);
    let inputs = urlArray.slice(2)
    console.log(inputs);
...
function postTest(response, request) {
        ...
        if (err) {
            ...
            fs.writeFile("./users.json", body, (err) => {
                ...
                if (err) {
                    ...
                    console.log("file saved!");
                    writeResponse(response, 200,JSON.stringify({result: "user created"}))
                }
            });
        }
    });
        ...
        if (err) {
            ...
            for (let item of body.records) {
                ...
                if (item.user == request.body.user && item.pass == request.body.pass) {
                    ...
                    writeResponse(response, 200, JSON.stringify({result : "login done"}), {
