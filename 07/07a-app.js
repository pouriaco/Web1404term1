import { use, start, writeResponse } from "./07a-httpFramework.js"
import fs from 'fs'
        ...
        if (err) {
            ...
            for (let item of body.records) {
                ...
                if (item.user == request.body.user && item.pass == request.body.pass) {
                    ...
                    let token = jwt.sign(item.user , secret);
                    let palin = jwt.verify(token , secret);
                    writeResponse(response, 200, token)
